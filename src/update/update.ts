...
function readCurrentParameterNames(
  ...
  const instantiateClassUnderTestNode = findNodes(
    ...
    node => node.kind === ts.SyntaxKind.NewExpression && node.getText().includes(classUnderTestName)
  ) as ts.NewExpression;
  ...
  // Array -> ['dep1', 'dep2']
  const parameterNames = findNodes(parametersList, ts.SyntaxKind.Identifier).map(n => n.getText());
...
function remove(toRemove: string[], setupFunction: ts.FunctionDeclaration, path: string) {
  ...
  return instantiations
    .concat(uses)
    .map(i => new RemoveChange(path, i.pos, i.getFullText()));
}

// @ts-ignore
function getTextPlusCommaIfAny(i: ts.Node) {
  const children = i.parent.getChildren();
  children.forEach(_printKindAndText);
